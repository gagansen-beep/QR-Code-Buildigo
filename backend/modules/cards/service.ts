...
import { query } from "../../middleware/database/connection";
...
export class CardService {
  ...
  /** Admin — get all cards (requires admin credentials) */
  static async adminGetAll(
    ...
    const offset = (page - 1) * limit;
    const countResult = await query<{ count: string }>("SELECT COUNT(*) FROM qr_cards");
    const total = Number(countResult.rows[0].count);
    const result = await query<QrCardRow>(
      "SELECT * FROM qr_cards ORDER BY created_at DESC LIMIT $1 OFFSET $2",
      [limit, offset],
    );
    return { data: result.rows.map(sanitizePublic), total, page, limit };
  }
  ...
  static async adminUpdate(
    ...
    password: string,
    id: string,
    input: { name?: string; designation?: string; phone?: string; website?: string; location?: string },
    newImageUrl?: string,
  ): Promise<QrCard> {
    verifyAdmin(email, password);
    const result = await query<QrCardRow>("SELECT * FROM qr_cards WHERE id = $1", [id]);
    if (!result.rows[0]) throw new NotFoundError("Card not found");
    const card = result.rows[0];
    const imageUrl = newImageUrl !== undefined ? newImageUrl : card.image_url;
    const updated = await query<QrCardRow>(
      `UPDATE qr_cards SET
         name=$1, designation=$2, image_url=$3, phone=$4, website=$5, location=$6,
         updated_at=NOW()
       WHERE id=$7
       RETURNING *`,
      [
        input.name?.trim() ?? card.name,
        input.designation !== undefined ? trim(input.designation) : card.designation,
        imageUrl,
        input.phone !== undefined ? trim(input.phone) : card.phone,
        input.website !== undefined ? trim(input.website) : card.website,
        input.location !== undefined ? trim(input.location) : card.location,
        id,
      ],
    );
    return sanitizePublic(updated.rows[0]);
  ...
  static async adminDelete(email: string, password: string, id: string): Promise<void> {
    verifyAdmin(email, password);
    await query("DELETE FROM qr_cards WHERE id = $1", [id]);
  }
}
