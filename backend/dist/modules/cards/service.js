...
class CardService {
    ...
    static async adminGetAll(email, password, page = 1, limit = 10) {
        verifyAdmin(email, password);
        const offset = (page - 1) * limit;
        const countResult = await (0, connection_1.query)("SELECT COUNT(*) FROM qr_cards");
        const total = Number(countResult.rows[0].count);
        const result = await (0, connection_1.query)("SELECT * FROM qr_cards ORDER BY created_at DESC LIMIT $1 OFFSET $2", [limit, offset]);
        return { data: result.rows.map(sanitizePublic), total, page, limit };
    }
    /** Admin — update any card */
    static async adminUpdate(email, password, id, input, newImageUrl) {
        verifyAdmin(email, password);
        const result = await (0, connection_1.query)("SELECT * FROM qr_cards WHERE id = $1", [id]);
        if (!result.rows[0])
            throw new NotFoundError("Card not found");
    ...
    static async adminDelete(email, password, id) {
        verifyAdmin(email, password);
        await (0, connection_1.query)("DELETE FROM qr_cards WHERE id = $1", [id]);
    }
}
