...
class CardController {
    ...
    static async getById(req, res, next) {
        try {
            const card = await service_1.CardService.findById(req.params.id);
            if (!card) {
    ...
    static async update(req, res, next) {
        try {
            ...
            const oldCard = await service_1.CardService.findById(req.params.id);
            if (!oldCard) {
                cleanupUpload(req.file);
                res.status(404).json({ success: false, message: "Card not found" });
                return;
            ...
            const newImageUrl = req.file
                ...
                : undefined;
            try {
                const card = await service_1.CardService.publicUpdate(req.params.id, {
                    name: req.body.name,
    ...
    static async remove(req, res, next) {
        try {
            ...
            await service_1.CardService.publicDelete(req.params.id);
    ...
    static async adminUpdate(req, res, next) {
        try {
            ...
            const newImageUrl = req.file
                ...
                : undefined;
            try {
                const card = await service_1.CardService.adminUpdate(body.admin_email, body.admin_password, req.params.id, {
                    name: body.name,
                    designation: body.designation,
                    phone: body.phone,
                    website: body.website,
    ...
    static async adminRemove(req, res, next) {
        try {
            ...
            const oldCard = await service_1.CardService.findById(req.params.id);
            await service_1.CardService.adminDelete(admin_email, admin_password, req.params.id);
            if (oldCard?.image_url)
                deleteOldImage(oldCard.image_url);
            res.json({ success: true, message: "Card deleted successfully" });
        }
...
class ContactController {
    ...
    static async getByIdContact(req, res, next) {
        try {
            const contact = await service_1.contactUsService.findById(req.params.id);
            if (!contact) {
    ...
    static async deleteContact(req, res, next) {
        try {
            await service_1.contactUsService.delete(req.params.id);
